...
server = function(input, output, session) {
  ...
  output$select_tf = renderUI({
    if (!is.null(input$select_contrast)) {
      ...
      default_selected = rownames(dorothea_result)[which(dorothea_result[, input$select_contrast] ==
  ...
  output$dorothea_result = DT::renderDataTable({
    results_confidence = unique.data.frame(merge(
      dorothea_hs[, c("tf", "confidence")],
      dorothea_result %>% round(digits = 3) %>% rownames_to_column(var = "tf"),
      by = "tf"
    ))
