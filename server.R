server = function(input, output, session) {
  ...
  output$select_pathway = renderUI({
    if (!is.null(input$select_contrast)) {
      ...
      default_selected = rownames(progeny_result)[which(progeny_result[, input$select_contrast] ==
  ...
  output$download_progeny_analysis = downloadHandler(
    ...
    content = function(x) {
      ...
      fnames = c(
        paste0("barplot_progeny_", input$select_contrast, ".png"),
        paste0(
